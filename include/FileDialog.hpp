...
class FileDialog {
  ...
  public:
    ...
    int OpenFileDialog(std::string& outPath, const nfdu8filteritem_t* filter = nullptr,
                       const size_t filterCount = 0);
    int SaveFileDialog(std::string& outPath, const nfdu8filteritem_t* filter = nullptr,
                       const size_t filterCount = 0);
};
